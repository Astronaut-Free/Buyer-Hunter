  ...
  async function runBuyerMessage(payload = {}, user) {
    ...
    try {
      ...
      dependencyExecutions.forEach((result, index) => {
        ...
        state.traces.push({ trace_id: id('trace'), run_id: run.run_id, span_type: 'CapabilitySpan',
          payload: { capability_id: result.capability_id, run_status: result.run_status }, timestamp: now() });
      });
      ...
      state.steps[step.step_id] = step;
      state.traces.push({ trace_id: id('trace'), run_id: run.run_id, span_type: 'CapabilitySpan',
        payload: { capability_id: step.capability_id, run_status: step.status }, timestamp: now() });
