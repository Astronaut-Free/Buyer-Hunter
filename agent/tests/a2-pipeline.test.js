...
test('A2 provider pipeline discovers company then contact and reaches READY', async () => {
  ...
  const result = await pipeline({
    ...
    providers: {
      trade_data: { async searchBuyers() { calls.push('trade'); return { companies: [{ buyer_company_id: 'c1', domain: 'buyer.example', sells_or_uses_product: true, product_evidence: ['ev_product'], trade_evidence: ['ev_trade'], evidence_refs: ['ev_company'] }] }; } },
