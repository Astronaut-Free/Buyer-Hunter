...
→ all_platform_records_cleaned.csv（184 条）
→ useful_current_opportunities.csv（30 条）
→ supporting_evidence.csv（24 条）
```

本链路负责回答“全量抓了多少、清洗后剩多少、哪些是当前机会、哪些只能当佐证”。其中 30 条当前机会的买方法定主体均未完成解析，因此 `formally_qualified_opportunities.csv` 为 0 条；联系人不会被冒充为买家公司。

### B. SQLite 机会决策与 Demo API

```text
buyer_signals_qualified.csv（默认输入快照 21 条）
...
`useful_current_opportunities.csv` 的 30 条记录尚未自动转换成 `build_opportunity_store_v1.py` 需要的完整事实字段，因此当前 API 仍使用 21 条默认输入快照。禁止把两批数字相加，也禁止宣称全平台 30 条已经进入运行数据库。

下一步适配必须保留以下原则：

- `record_id` 映射为稳定 `signal_id`，不能只用共享 API URL 去重；
- `contact_person_raw` 不得映射为 `buyer.canonical_name`；
- 缺失的四维真实性子分不得由总分反推或平均分配；
- `HISTORICAL_PURCHASE` 和 `BUYER_BACKGROUND` 只能进入证据层，不能生成当前机会；
- 官方采购没有未来截止时间时只能作为佐证；
- 适配完成后更新 API 测试的输入总数和品类筛选预期。
